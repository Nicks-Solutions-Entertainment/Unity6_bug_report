...
/// </remarks>
[AddComponentMenu("Input/On-Screen DragZone")]
//[HelpURL(InputSystem.kDocUrl + "/manual/OnScreen.html#on-screen-sticks")]
public class OnScreenDragZone : OnScreenControl, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    private const string kDynamicOriginClickable = "DynamicOriginClickable";
    Vector2 _currentFrameInput = Vector2.zero;
    Vector2 _lastFrameInput = Vector2.zero;
    /// <summary>
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        MoveStick(eventData.delta, eventData.pressEventCamera);
    }
    ...
    {
        if (_currentFrameInput.magnitude > 0 && _lastFrameInput == _currentFrameInput)
        {
    ...
    private void MoveStick(Vector2 pointerDrag, Camera uiCamera)
    {
        var canvasRect = transform.parent?.GetComponentInParent<RectTransform>();
        if (canvasRect == null)
        {
        ...
        //RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, pointerDrag, uiCamera, out var position);
        ...
        //if (pointerDrag == _lastFrameInput)
        //    pointerDrag = Vector2.zero;

        SendValueToControl(pointerDrag);
        _lastFrameInput = pointerDrag;

        _currentFrameInput = pointerDrag;
    }

    void RegisterFrameInput(Vector2 input = default)=> _currentFrameInput = input;

    protected TValue ReadValueOnControl<TValue>()
            where TValue : struct
    {
        if (control == null)
            return default(TValue);

        if (!(control is InputControl<TValue> _control))
            throw new ArgumentException(
                $"The control path {controlPath} yields a control of type {control.GetType().Name} which is not an InputControl with value type {typeof(TValue).Name}");

        TValue _value = _control.ReadValue();
        return _value;
    }
    ...
    private void OnPointerMove(InputAction.CallbackContext ctx)
    {
        ...
        var screenDelta = ((Pointer)ctx.control.device).delta.ReadValue();

        MoveStick(screenDelta, GetCameraFromCanvas());
    }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(OnScreenDragZone))]
    internal class OnScreenDragZoneEditor : UnityEditor.Editor
    {
        ...
        public void OnEnable()
        {
            ...
            m_UseIsolatedInputActions = serializedObject.FindProperty(nameof(OnScreenDragZone.m_UseIsolatedInputActions));

            m_Behaviour = serializedObject.FindProperty(nameof(OnScreenDragZone.m_Behaviour));
            m_ControlPathInternal = serializedObject.FindProperty(nameof(OnScreenDragZone.m_ControlPath));
            //m_MovementRange = serializedObject.FindProperty(nameof(OnScreenDragZone.m_MovementRange));
            m_DynamicOriginRange = serializedObject.FindProperty(nameof(OnScreenDragZone.m_DynamicOriginRange));
            m_PointerDownAction = serializedObject.FindProperty(nameof(OnScreenDragZone.m_PointerDownAction));
            m_PointerMoveAction = serializedObject.FindProperty(nameof(OnScreenDragZone.m_PointerMoveAction));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_ShowDynamicOriginOptions.target = ((OnScreenDragZone)target).behaviour ==
            ...
            if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded))
            {
                ...
                {
                    ((OnScreenDragZone)target).UpdateDynamicOriginClickableArea();
                }
                EditorGUI.indentLevel--;
            }
